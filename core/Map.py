...
def build_corridor_map(
    ...
    *,
    ...
    # Map layout with better defaults; uirevision preserves user zoom
    mapbox_cfg = dict(
        style=map_style,
...
def build_intersections_overview(
    ...
    *,
    ...
    map_style: str = "carto-positron",
    keep_view: bool = True,
    cluster: bool = True,  # try cluster for readability at low zoom
) -> Optional[go.Figure]:
    """
    ...
    if oth_lat:
        ...
        if cluster:
            # Plotly supports clustering on Scattermapbox in recent versions
            try:
