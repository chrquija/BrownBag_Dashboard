...
# ---------- THEME-AWARE CSS (new) ----------
def _inject_cycle_css():
    """
    Forces good contrast in dark mode for KPI titles/captions and the legend title.
    Safe to call multiple times.
    """
    st.markdown(
        """
        <style>
          /* KPI cards / metrics – titles */
          html[data-theme="dark"] [data-testid="stMetricLabel"],
          html[data-base-theme="dark"] [data-testid="stMetricLabel"],
          body[data-theme="dark"] [data-testid="stMetricLabel"]{
            color: #e6f0ff !important;  /* light blue-white */
          }

          /* KPI captions you render with st.caption() */
          html[data-theme="dark"] .kpi-caption,
          html[data-base-theme="dark"] .kpi-caption,
          body[data-theme="dark"] .kpi-caption{
            color: rgba(255,255,255,.85) !important;
          }

          /* Extra safety: make any caption text inside KPI columns readable */
          html[data-theme="dark"] [data-testid="stVerticalBlock"] .st-emotion-cache-1h7q8he p,
          html[data-base-theme="dark"] [data-testid="stVerticalBlock"] .st-emotion-cache-1h7q8he p{
            color: rgba(255,255,255,.85) !important;
          }

          /* Legend box header */
          .cycle-legend .legend-title{
            color: #1e3c72; /* light mode */
            font-weight: 700;
            margin-bottom: .35rem;
          }
          html[data-theme="dark"] .cycle-legend .legend-title,
          html[data-base-theme="dark"] .cycle-legend .legend-title{
            color: #cfe8ff !important; /* high-contrast in dark mode */
          }
        </style>
        """,
...
def _legend_html() -> str:
    """HTML legend for cycle length thresholds (now with theme-safe title)."""
    return (
        ...
        '<div class="legend-title">Cycle Length Thresholds</div>'
...
def render_cycle_length_section(raw: pd.DataFrame, key_prefix: str = "cycle") -> None:
    ...
    with k1:
        ...
        st.caption('<span class="kpi-caption"></span>', unsafe_allow_html=True)
    with k2:
    ...
    with k4:
        ...
        st.caption(f'<span class="kpi-caption">Threshold: &gt; {HIGH_VOLUME_THRESHOLD_VPH:,} vph</span>', unsafe_allow_html=True)
        st.caption(f'<span class="kpi-caption">Hours: {_hours_preview(exceed_hour_labels)}</span>', unsafe_allow_html=True)
    with k5:
        st.metric("🚦 Peak Capacity Utilization", f"{peak_capacity_util:.0f}%", delta=f"Peak {peak_volume_pd:,.0f} vph")
        st.caption(f'<span class="kpi-caption">Capacity: {INTERSECTION_CAPACITY_VPH:,} vph</span>', unsafe_allow_html=True)
